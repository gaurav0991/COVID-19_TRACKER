...
function App() {
  ...
  useEffect(() => {
    fetch(`${BASE_URL.BASE_URL}/all`)
      .then((response) => response.json())
      .then((d) => setSelecetedCountryInfo(d));
  }, []);
  useEffect(() => {
    const getData = async () => {
      await fetch(`${BASE_URL.BASE_URL}/countries`)
        .then((response) => response.json())
        .then((data) => {
  ...
  const onCountryChange = async (event) => {
    ...
    const url =
      event.target.value == "worldwide"
        ...
        : `${BASE_URL.BASE_URL}/countries/${event.target.value}`;
    await fetch(url)
      .then((response) => response.json())
      .then((data) => {
        setSelecetedCountry(event.target.value);
      ...
      <div className="app_main">
        ...
        <div className="app_box">
          <Box
            ...
            title="Corona Cases"
            cases={
              selectedCountryInfo.todayCases
                ? `+${numeral(selectedCountryInfo.todayCases).format("0.0a")}`
                : "+0"
            }
          ...
          <Box
            ...
            title="Recovered "
            cases={
              selectedCountryInfo.todayRecovered
                ? `+${numeral(selectedCountryInfo.todayRecovered).format(
                    "0.0a"
                  )}`
                : "+0"
            }
          ...
          <Box
            ...
            title="Deaths"
            cases={
              selectedCountryInfo.todayDeaths
                ? `+${numeral(selectedCountryInfo.todayDeaths).format("0.0a")}`
                : "+0"
            }
